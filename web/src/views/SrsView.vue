...
function next() {
  // Ошибся — карточка возвращается в конец сессии, чтобы повторить её сегодня.
  if (grade.value && !grade.value.correct) exam.requeueCurrent();
  checked.value = false;
  exam.next();
}
...
<template>
  <div v-if="setupOpen">
    ...
    <p class="muted">
      Ответ проверяется автоматически. По алгоритму SM-2 правильно отвеченный
      вопрос вернётся через 1, 3, 7, 14, 30+ дней. Ошибся — карточка вернётся
      в конце текущей сессии, чтобы повторить её сегодня же.
    </p>
    <div class="card">
