...
function check() {
  ...
  if (g.correct) progress.clearMistake(q.id);
}
...
<template>
  <div v-if="setupOpen">
    ...
    <p class="muted">
      Здесь собраны все вопросы, в которых вы ошибались. После правильного ответа вопрос
      автоматически выводится из этого списка.
    </p>
    <div class="card">
      <p>Вопросов в работе над ошибками: <strong>{{ mistakeQuestions.length }}</strong></p>
      <button class="btn primary" :disabled="mistakeQuestions.length === 0" @click="start">
        Начать
      </button>
    </div>
  </div>
  ...
  <div v-else-if="exam.finished">
    <h1>Готово!</h1>
    <p>Все ошибки разобраны.</p>
    <div class="btn-row">
      <button class="btn primary" @click="back">К списку</button>
  ...
  <div v-else-if="exam.current">
    ...
    <div v-if="grade" class="card" :style="{ borderColor: grade.correct ? 'var(--success)' : 'var(--error)' }">
      ...
      <strong v-else class="error">✗ Снова неправильно</strong>
